...
def huskyOdomCallback(data, args):
	...
	pub, cmd_msg, goal, trajectory, measured_pos, distance, t_total, t0, a = args


	# Increase x a tiny bit to make line
	...
	# Add data to lists for plotting later
	measured_pos.append([x,y])
	trajectory.append([goal[0],goal[1]])
	t_total.append(time)
	distance.append(round(ro,3))

	print(ro)
	
	# Get 20000 samples before plotting
...
def setup():
	...
	modelstate_subscriber = rospy.Subscriber('gazebo/model_states',  ModelStates,  huskyOdomCallback, (pub, cmd_msg, goal, trajectory, measured_pos, distance, t_total, t0, a))
